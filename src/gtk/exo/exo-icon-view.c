...
#if GTK_CHECK_VERSION(3, 0, 0)
  event_area.x = event_area.y = 0;
  event_area.width = gtk_widget_get_allocated_width (widget);
  event_area.height = gtk_widget_get_allocated_height (widget);
#else
                                   ...
                                   gint          y)
{
  ...
  /* save the color to draw the rubberband background */
  ...
  if (G_LIKELY (icon_view->priv->doing_rubberband))
    {
      ...
      /* drop the colors for drawing the rubberband */
