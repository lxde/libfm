...
static void add_place_holder_child_item(FmDirTreeModel* model, GList* parent_l, GtkTreePath* parent_tp, gboolean emit_signal)
{
    ...
    if(emit_signal)
    {
        ...
        gtk_tree_path_append_index(parent_tp, 0);
        gtk_tree_model_row_inserted(GTK_TREE_MODEL(model), parent_tp, &it);
        gtk_tree_path_up(parent_tp);
    }
}
...
static void remove_item(FmDirTreeModel* model, GList* item_l)
{
    ...
    if(item)
    {
        if(parent_item)
        {
            ...
            if(parent_item->children->next == NULL)
            {
...
static void on_folder_files_removed(FmFolder* folder, GSList* files, GList* item_l)
{
    ...
    for(l = files; l; l = l->next)
    {
        FmFileInfo* fi = FM_FILE_INFO(l->data);
        FmPath* path = fm_file_info_get_path(fi);
        GList* rm_item_l = children_by_name(model, item->children, path->name, NULL);
        if(rm_item_l)
...
static void on_folder_files_changed(FmFolder* folder, GSList* files, GList* item_l)
{
    ...
    for(l = files; l; l = l->next)
    {
        ...
        int idx;
        FmPath* path = fm_file_info_get_path(fi);
        GList* changed_item_l = children_by_name(model, item->children, path->name, &idx);
